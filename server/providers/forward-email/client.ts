...
/**
 * One record as Forward Email publishes it for this specific domain.
 *
 * `name` arrives relative (`_dmarc`, `fe-bounces`, `fe-5687bdb44e._domainkey`)
 * which is what every DNS host's "name" field actually wants, so it is used
 * verbatim rather than being expanded to a FQDN.
 */
export interface ForwardEmailSmtpRecord {
  name?: string;
  value?: string;
}
...
export interface ForwardEmailDomain {
  ...
  has_strict_dmarc?: boolean;
  /**
   * The DKIM, return-path and DMARC records required for *sending*.
   *
   * These are per-domain and generated by the provider — the DKIM public key
   * and its selector exist nowhere else, and the DMARC `rua` names a provider
   * mailbox keyed to the domain id. Reconstructing them from a template is how
   * a dashboard ends up showing plausible values that are not the ones the
   * operator has to publish, which leaves them opening the provider's own
   * console to copy the real thing.
   */
  smtp_dns_records?: {
    dkim?: ForwardEmailSmtpRecord;
    return_path?: ForwardEmailSmtpRecord;
    dmarc?: ForwardEmailSmtpRecord;
  };
